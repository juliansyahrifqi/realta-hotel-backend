...
export class WorkOrdersService {
  ...
  async findAll(page: number, limit: number): Promise<any> {
    try {
      ...
      const result = await this.workOrdersModel.findAll({
        attributes: ['work_start_date', 'woro_status'],
        include: [
          {
            model: users,
            attributes: ['user_id'], // ganti user_full_name
          },
        ],
        offset,
        limit,
  ...
  async findOne(work_start_date: Date, from: Date, to: Date, status: string) {
    try {
      const where: any = {};
      if (status) {
      ...
      const result = await this.workOrdersModel.findOne({ where });
      if (!result) {
        return {
          message: `Tidak ada work order yang ditemukan pada tanggal ${work_start_date} dan dalam rentang waktu dari ${from} hingga ${to} dengan status ${status}!`,
        };
      }
      return {
        message: `Ditemukan ${result.length} work order pada tanggal ${work_start_date} dan dalam rentang waktu dari ${from} hingga ${to} dengan status ${status}.`,
        data: result,
      };
    } catch (error) {
      return error;
